...
from distill.criterion import DistillKL,  kl_loss
from distill.dist_loss import DIST_LOSS
from dataset.transform_cfg import transforms_options, transforms_list

from util import adjust_learning_rate, accuracy, AverageMeter, rotrate_concat, \
    Meter, generate_final_report, restart_from_checkpoint, set_gpu, mix_data_lab, record_data
from eval.meta_eval import meta_test

from losses import  mixup_supcontrastive_loss
from modules.loss import kl_loss, mse_loss, SupCluLoss
from eval.extract_feat import extract_features
from dataloader import get_train_dataloader, get_eval_dataloader,get_aux_dataloader
...
def parse_option():
    ...
    parser.add_argument('--tags', type=str, default="gen1, ce", help='add tags for the experiment')
    ...
    parser.add_argument('--path_t', type=str,
                        default="./save/~/ckpt_epoch_90.pth",
    ...
    parser.add_argument('--dist_w', type=float, default=1.5, help='loss coefficient for rd loss')
    ...
    parser.add_argument('--trans_type', type=str, default='rot_color_perm12',
                        help='rotation,rotation2,color_perm6,color_perm3,rot_color_perm6,'
    ...
    opt.model_name = 'S:{}_T:{}_{}_{}_r:{}_gw_{}_a:{}_dist:{}_gT_:{}_Lt:{}_mixT:{}_type:{}'.format(opt.model_s,opt.model_t,opt.dataset,opt.distill,opt.gamma,opt.global_w, opt.alpha,
                                                                                                   opt.dist_w,opt.global_t,opt.local_t,opt.mix_t,opt.trans_type)
    if opt.cosine:
...
def main():
    ...
    # dataloader
    # meta_testloader, meta_valloader, n_cls, no_sample,dataset = get_dataloaders(opt)
    ...
    train_dataset, train_aux_loader = get_aux_dataloader(opt)
    ...
    print('==> Initialize prototype class centroids in the center memory')
    sup_feature, _ = extract_features(teacher, train_aux_loader, print_freq=50)
    sup_fea_dict = collections.defaultdict(list)
    for img, label, fname in (train_dataset.img_label):
        sup_fea_dict[label].append(sup_feature[fname].unsqueeze(0))
    cls_centers = [torch.cat(sup_fea_dict[label], 0).mean(0) for label in (sup_fea_dict.keys())]
    cls_centers = torch.stack(cls_centers, 0)
    cls_centers = F.normalize(cls_centers, dim=1)
    ...
    for epoch in range(start_epoch, opt.epochs + 1):
        ...
        time1 = time.time()
        loss, top1, top5 = train(epoch, train_loader, student, teacher,cls_centers,
                                 criterion_supclu_l, criterion_supclu_g, optimizer, opt)
        time2 = time.time()
...
def train(epoch, train_loader, model_s, model_t,cls_centers, criterion_supclu_l, criterion_supclu_g,optimizer, opt):
    """One epoch training"""
    model_s.train()
    model_t.eval()
    losses = Meter()
    ...
    for iter, (jig_images, geo_images, targets, indices) in enumerate(tqdm_gen):
        ...
        # global_data = torch.cat([geo_images[0],geo_images[1]],dim=0)
        batch_size = geo_images[0].size(0)
        input_data = torch.cat(geo_images,dim=0)
        ...
        # forward compute
        feat,_,m_logits,out_s = model_s(aug_data)
        ...
        with torch.no_grad():
            ...
            local_feat_t = model_t.local_head(mix_clu_gather_t)
        ...
        logits = torch.mm(feat,cls_centers.t())/opt.tau
        ce_loss = F.cross_entropy(logits, targets)
        #
        feat_t = F.normalize(feat_t,dim=-1)
        logits_t = torch.mm(feat_t,cls_centers.t())/opt.tau
        ...
        skd_loss = F.kl_div(F.log_softmax(logits / opt.distill_t, 1),
                                     F.softmax(agg_preds.detach() / opt.distill_t, 1),
                                     reduction='batchmean')
        sup_loss = ce_loss + joint_loss + skd_loss.mul(opt.distill_t ** 2)
        ...
        if opt.use_global_feat:
            if n == 12 or n == 24:
                ...
                global_num = n
            mix_loss= mixup_supcontrastive_loss(mix_feat, targets,opt.mix_t, batch_size, index1, index2, lam1, lam2,out_t_subfeat,global_num)
        else:
            mix_loss= mixup_supcontrastive_loss(mix_feat, targets,opt.mix_t, batch_size, index1, index2, lam1, lam2)

        # compute distillation loss
        distill_feat_dist = DIST_LOSS(mix_feat, mix_feat_t) + DIST_LOSS(local_feat_norm, local_feat_t_norm) + DIST_LOSS(out_s, out_t)
        distill_logits = kl_loss(logits, logits_t)

        distill_loss = (distill_logits + distill_feat_dist * opt.dist_w) * opt.alpha
        loss = sup_loss + local_agg_loss + (mix_loss + global_agg_loss) * opt.global_w + distill_loss
