...
def parse_option():
    ...
    parser.add_argument('--tags', type=str, default="gen0, hcrd", help='add tags for the experiment')
    ...
    opt.model_name = '{}_{}_lr_{}_decay_{}_a_{}_supT_{}_mixT_{}_augtype_{}_trans_{}'.format(opt.model, opt.dataset, opt.learning_rate, opt.weight_decay,
                                                            opt.alpha,opt.sup_t,opt.mix_t,opt.trans_type,opt.transform)
    if opt.cosine:
    ...
    opt.record_file = os.path.join(opt.record_path,opt.trans_type + opt.trial+'.csv')
...
def main():
    ...
    for epoch in range(start_epoch, opt.epochs+1):
        ...
        if feat_meta_test_acc > trlog['max_acc']:
            ...
            torch.save({'epoch': epoch, 'model': model_s.state_dict(),'model_t':model_t.state_dict()}, outfile)
