...
class ResNet(nn.Module):
    ...
    def __init__(self, block, n_blocks, keep_prob=1.0, avg_pool=False, clu_avg_pool=True, drop_rate=0.0,
                 dropblock_size=5, num_classes=-1, embd_size=128, num_trans=-1, use_se=False):
        ...
        self.clu_avg_pool = clu_avg_pool
        self.dropout = nn.Dropout(p=1 - self.keep_prob, inplace=False)
        self.drop_rate = drop_rate
        self.trans = num_trans
        ...
        if self.trans > 0:
            self.mutil_classifier = nn.Linear(640, num_trans * num_classes)
        else:
            self.mutil_classifier = nn.Identity()
    ...
    def forward(self, x, is_feat=False, use_clu=False):
        ...
        if is_feat:
            ...
            m_logits = self.mutil_classifier(feat)
            return feat,logits, m_logits, out
