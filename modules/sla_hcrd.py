...
class HCRD(nn.Module):
    ...
    def local_forward(self,jig_images,targets):
        ...
        with torch.no_grad():
            ...
            local_feat_t_norm = nn.functional.normalize(local_feat_t,dim=1)
        distill_loss = self.dist_loss(local_feat_norm,local_feat_t_norm)
    ...
    def forward(self,aug_data,data_x,x_mix=None,target=None):
        img_k = data_x[1]
        batch_size = img_k.size(0)
        n = aug_data.size(0) // batch_size
        _,logits,m_logits,out_s = self.encoder_q(aug_data)

        with torch.no_grad():
            _,logits_t,m_logits_t,out_t = self.encoder_k(aug_data)


        feat_distill_dist = self.dist_loss(out_s,out_t)
        logit_distill_loss = self.kl_loss(logits,logits_t) + self.kl_loss(m_logits,m_logits_t)

        loss_local, local_distll_loss = self.local_forward(x_mix, target)
        #
        if self.use_global_feat:
            if n == 12 or n == 24:
                ...
                global_num = n
            mix_loss, mix_distill_loss = self.mixup_forward(data_x, target, out_s_subfeat,global_num)
        else:
            mix_loss, mix_distill_loss = self.mixup_forward(data_x, target)

        multi_labels = torch.stack([target*n+i for i in range(n)], 1).view(-1)
        logits = logits[::n]
        joint_loss = F.cross_entropy(m_logits,multi_labels)
        single_loss = F.cross_entropy(logits,target)
        #
        agg_preds = 0
        for i in range(n):
            agg_preds = agg_preds + m_logits[i::n, i::n] / n
        #
        distillation_loss = F.kl_div(F.log_softmax(logits / self.soft_t, 1),
                                     F.softmax(agg_preds.detach() / self.soft_t, 1),
                                     reduction='batchmean')

        sup_loss = joint_loss+single_loss + distillation_loss.mul(self.soft_t**2)
        # global aggregation loss
        global_labels = torch.stack([target for i in range(n)], 1).view(-1)
        global_agg_loss = self.criterion_clu(out_s,global_labels)

        distill_loss = ( mix_distill_loss + local_distll_loss + logit_distill_loss + feat_distill_dist) * self.alpha

        loss = sup_loss + global_agg_loss + loss_local + mix_loss + distill_loss
    ...
    def mixup_forward(self,data_x,target,sub_feat=None,sub_num=None):
        ...
        mix_distill_loss = self.dist_loss(mix_feature,mix_feature_t)
        ...
        return loss.mean(),mix_distill_loss
