...
def parse_option():
    ...
    parser.add_argument('--tags', type=str, default="gen1, hsc", help='add tags for the experiment')
    ...
    parser.add_argument('--global_w',type=float,default=0.1,help='loss coefficient for global supervised loss ')

    # memory hyper parameters
    parser.add_argument('--use_global_feat', type=bool, default=False,
    ...
    parser.add_argument('--tea_trans', type=int, default=2)
    ...
    opt.model_name = 'S:{}_T:{}_{}_{}_r:{}_gw_{}_a:{}_dist:{}_gT_:{}_Lt:{}_mixT:{}_tag_{}'.format(opt.model_s,opt.model_t,opt.dataset,opt.distill,opt.gamma,opt.global_w, opt.alpha,
                                                                                                   opt.dist_w,opt.global_t,opt.local_t,opt.mix_t,
                                                                                                   opt.tags[-1])
    if opt.cosine:
...
def train(epoch, train_loader, model_s, model_t, criterion_supclu_l, criterion_supclu_g,optimizer, opt):
    """One epoch training"""
    model_s.train()
    model_t.eval()
    losses = Meter()
    ...
    for iter, (jig_images, geo_images, targets, indices) in enumerate(tqdm_gen):
        ...
        # forward compute
        feat,logits, _, out_s = model_s(input_data)
        mix_feat,_, _, mix_feat = model_s(mix_data)
        ...
        with torch.no_grad():
            ...
            local_clu_feat_t = F.normalize(local_clu_gather_t,dim=-1)
        ...
        global_agg_loss = criterion_supclu_g(out_s, targets)
        mix_loss = mixup_supcontrastive_loss(mix_feat, o_targets, opt.mix_t, batch_size, index1, index2, lam1, lam2)

        # compute distillation loss
        loss_rd = DIST_LOSS(mix_feat, mix_feat_t) + DIST_LOSS(local_feat_norm, local_feat_t_norm) + DIST_LOSS(out_s, out_t)

        loss_nld = kl_loss(logits, logits_t)
        distill_loss = (loss_nld + loss_rd * opt.dist_w) * opt.alpha

        loss = ce_loss + local_agg_loss + mix_loss + global_agg_loss + distill_loss
